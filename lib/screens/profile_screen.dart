...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void initState() {
    super.initState();
    _profileFormProvider = ProfileFormProvider();

    // Llamar _photoExist después de que el widget se haya construido
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _initializePhotoCheck();
    });
  }

  Future<void> _initializePhotoCheck() async {
    await _photoExist();
  }
            ...
            child: SingleChildScrollView(
              child: Container(
                ...
                child: ChangeNotifierProvider.value(
                  value:
                      _profileFormProvider, // Usar .value para la instancia existente
                  child: _ProfileForm(
  ...
  Future<void> _photoExist() async {
    ...
    try {
      if (authProvider.user?.photo != null && authProvider.user?.photo != '') {
        ...
        final exist = jsonDecode(existFile.body)['exist'] ?? false;        
        _profileFormProvider.photoExists = exist;        
      }
    } catch (e) {
      if (mounted) {
...
class _ProfileForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    final profileForm = Provider.of<ProfileFormProvider>(context, listen: true);
                  ...
                  child: SizedBox(
                    ...
                    child: MouseRegion(
                      ...
                      child: GestureDetector(
                        onTap: () => isEditing ? _onUploadImage(context) : null,
                        child: profileForm.photoExists
                            ? Center(
                                child: Stack(
                                  children: [
                                    // CircleAvatar base
  ...
  Future<void> userPatch(BuildContext context) async {
    ...
    await userService.userPatchProfile(
      UserResponse(
        ...
        rolId: authProvider.user?.rolId ?? authProvider.user?.rol.id ?? '1',
